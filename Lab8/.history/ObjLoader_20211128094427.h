...
public:
    ...
    void Sweeping(float radius, float bendRadius) {
        ...
        int slices = 8;
        GLfloat bend_ang0, bend_ang1; 
        bend_ang0  = -bendRadius * 1.0f / 2.0f;
        bend_ang1  = bendRadius * 1.0f / 2.0f;
 
        for(int i = 0; i < 20; i++) {
            float w0 = (float)i / 20;
            float w1 = (float)(i+1) / 20;
        
            ang0 = -bendRadius * 1.0f / 2.0f + (bendRadius * 1.0f / 2.0f + bendRadius * 1.0f / 2.0f) * w0;
            ang1 = -bendRadius * 1.0f / 2.0f + (bendRadius * 1.0f / 2.0f + bendRadius * 1.0f / 2.0f) * w1;
