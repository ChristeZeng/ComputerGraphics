...
void GetInputKey(unsigned char key, int x, int y) {
    if(key == 'w') {
        //float cosa = (eyez - centerz) / sqrt((eyex - centerx) * (eyex - centerx) + (eyez - centerz) * (eyez - centerz));
        //float sina = (eyex - centerx) / sqrt((eyex - centerx) * (eyex - centerx) + (eyez - centerz) * (eyez - centerz));
        eyez -= (eyez - centerz) * 0.1;
        eyex += (eyex - centerx) * 0.1;
    }
    else if(key == 's') {
        eyez += (eyez - centerz) * 0.1;
        eyex -= (eyex - centerx) * 0.1;
